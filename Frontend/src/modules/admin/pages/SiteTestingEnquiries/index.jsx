...
import { 
  ...
  FiCrosshair, FiClock, FiActivity, FiShield
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';

const SiteTestingEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      setLoading(true);
      const res = await api.get('/admin/sitetesting-enquiries');
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/admin/sitetesting-enquiries/${id}`);
      if (res.data.success) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/sitetesting-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Testing Scheduled': return 'bg-amber-100 text-amber-800 border-amber-200';
      case 'Testing In Progress': return 'bg-blue-100 text-blue-800 border-blue-200';
      case 'Report Generating': return 'bg-indigo-100 text-indigo-800 border-indigo-200';
      case 'Report Sent': return 'bg-green-100 text-green-800 border-green-200';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <FiCrosshair className="text-amber-500" />
            Site Testing & Audit Enquiries
          </h1>
          <p className="text-gray-500 mt-1">Manage all security system testing and inspection requests</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">
            ...
            <p>No site testing enquiries found.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredEnquiries.map((enq) => (
                  <tr key={enq._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="text-sm text-gray-800">{enq.siteType}</div>
                      <div className="text-xs text-gray-500 truncate max-w-[150px]">{enq.city}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-medium text-gray-800 truncate max-w-[150px]">{enq.purposeOfTesting}</div>
                      <div className="text-xs text-amber-600">{enq.projectUrgency}</div>
                    </td>
                    <td className="p-4">
                      <select 
                        value={enq.status} 
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-800">Testing & Audit #{selectedEnquiry._id.slice(-6).toUpperCase()}</h2>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="border border-gray-100 rounded-xl p-5 shadow-sm">
                  <h3 className="text-md font-bold text-gray-800 mb-4 flex items-center gap-2"><FiMapPin className="text-amber-500"/> Client Information</h3>
                  <div className="space-y-3">
                    ...
                    {selectedEnquiry.companyName && <div><span className="text-xs text-gray-500 block">Company</span><div>{selectedEnquiry.companyName}</div></div>}
                    <div>
                    ...
                    {selectedEnquiry.designation && <div><span className="text-xs text-gray-500 block">Designation</span><div>{selectedEnquiry.designation}</div></div>}
                ...
                {/* 2. Site Details */}
                <div className="border border-gray-100 rounded-xl p-5 shadow-sm">
                  <h3 className="text-md font-bold text-gray-800 mb-4 flex items-center gap-2"><FiBox className="text-purple-500"/> Site Overview</h3>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                      <span className="text-xs text-gray-500 block">System State</span>
                      <div className="font-medium text-sm bg-purple-50 text-purple-700 px-3 py-2 rounded-lg mt-1 border border-purple-100">{selectedEnquiry.installationType}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-gray-100 rounded-xl p-5 shadow-sm md:col-span-2">
                  <h3 className="text-md font-bold text-gray-800 mb-4 flex items-center gap-2"><FiCrosshair className="text-red-500"/> Testing Requirements</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                      <span className="text-xs text-red-600 block">Purpose of Testing</span>
                      <div className="font-bold text-red-800">{selectedEnquiry.purposeOfTesting}</div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <span className="text-xs text-gray-500 block">Total Devices</span>
                      <div className="font-medium">{selectedEnquiry.totalPointsToTest} Points</div>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <span className="text-xs text-gray-500 block">Brand Focus</span>
                      <div className="font-medium">{selectedEnquiry.equipmentBrand}</div>
                    </div>
                  </div>

                  <div className="mb-4">
                    <span className="text-xs text-gray-500 block mb-2">Selected Tests</span>
                    <div className="flex flex-wrap gap-2">
                      {selectedEnquiry.testingType?.map(test => (
                        <span key={test} className="px-3 py-1 bg-amber-50 border border-amber-100 text-amber-800 text-xs rounded-full">{test}</span>
                      ))}
                    </div>
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
                    <div><span className="text-xs text-gray-500 block">Installed By</span><div className="text-sm font-medium">{selectedEnquiry.installedBy}</div></div>
                    <div><span className="text-xs text-gray-500 block">System State</span><div className="text-sm font-medium">{selectedEnquiry.systemWorkingStatus}</div></div>
                    <div><span className="text-xs text-gray-500 block">Blind Spot Check</span><div className="text-sm font-medium">{selectedEnquiry.needBlindSpotAnalysis}</div></div>
                    <div><span className="text-xs text-gray-500 block">Network Test</span><div className="text-sm font-medium">{selectedEnquiry.needNetworkTesting}</div></div>
                  </div>
                  
                  {selectedEnquiry.focusArea && (
                    <div className="mt-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
                      <span className="text-xs font-bold text-gray-700 block mb-1">Focus Areas</span>
                      <p className="text-sm text-gray-600">{selectedEnquiry.focusArea}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="border border-gray-100 rounded-xl p-5 shadow-sm">
                  <h3 className="text-md font-bold text-gray-800 mb-4 flex items-center gap-2"><FiFileText className="text-blue-500"/> Reporting</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-sm text-gray-600">Need Written Report?</span>
                      <span className="text-sm font-medium">{selectedEnquiry.needWrittenReport}</span>
                    </div>
                    {selectedEnquiry.needWrittenReport === 'Yes' && (
                      <>
                        <div className="flex justify-between border-b border-gray-50 pb-2">
                          <span className="text-sm text-gray-600">Format</span>
                          <span className="text-sm font-medium">{selectedEnquiry.reportFormat}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-50 pb-2">
                          <span className="text-sm text-gray-600">Photo Evidence</span>
                          <span className="text-sm font-medium">{selectedEnquiry.needPhotoEvidence}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-50 pb-2">
                          <span className="text-sm text-gray-600">Pass/Fail Status</span>
                          <span className="text-sm font-medium">{selectedEnquiry.needPassFailStatus}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-50 pb-2">
                          <span className="text-sm text-gray-600">Compliance Audit</span>
                          <span className="text-sm font-medium">{selectedEnquiry.reportForCompliance}</span>
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div className="border border-gray-100 rounded-xl p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 bg-green-50 p-3 rounded-lg border border-green-100">
                      ...
                      <div className="font-bold text-green-800">{selectedEnquiry.budgetRange}</div>
                    </div>
                    ...
                    <div><span className="text-xs text-gray-500 block">Time Slot</span><div className="font-medium text-sm">{selectedEnquiry.preferredTimeSlot}</div></div>
                    {selectedEnquiry.preferredVisitDate && (
                      <div className="col-span-2"><span className="text-xs text-gray-500 block">Preferred Date</span><div className="font-medium text-sm">{new Date(selectedEnquiry.preferredVisitDate).toLocaleDateString()}</div></div>
                    )}
                    <div className="col-span-2"><span className="text-xs text-gray-500 block">Access Restrictions</span><div className="text-sm font-medium text-red-600">{selectedEnquiry.accessRestrictions}</div></div>
                  </div>
                </div>
              ...
              {selectedEnquiry.attachments && selectedEnquiry.attachments.length > 0 && (
                <div className="mt-6 border border-gray-100 rounded-xl p-5 shadow-sm bg-gray-50">
                  <h3 className="text-md font-bold text-gray-800 mb-4">Attachments ({selectedEnquiry.attachments.length})</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {selectedEnquiry.attachments.map((file, i) => (
                      <a key={i} href={file.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-200 hover:border-amber-300 hover:shadow-md transition-all group">
                        <div className="w-10 h-10 bg-amber-50 text-amber-500 rounded-lg flex items-center justify-center shrink-0">
                          <FiDownload />
                        </div>
                        <div className="truncate text-sm font-medium text-gray-700 group-hover:text-amber-600 transition-colors">
...
export default SiteTestingEnquiries;
