...
const DcPowerServices = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/dcpowerservice-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/dcpowerservice-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">DC Power Services</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex items-center justify-between z-10">
              <h2 className="text-xl font-bold text-gray-800">DC Power Service Details</h2>
            ...
            <div className="p-6 space-y-8">
              ...
              {/* DC Details */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">DC System Details</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Site Type</p><p className="font-medium">{selectedEnquiry.siteType}</p></div>
                  <div><p className="text-gray-500">Brand</p><p className="font-medium">{selectedEnquiry.equipmentBrand}</p></div>
                  <div><p className="text-gray-500">Voltage System</p><p className="font-medium">{selectedEnquiry.dcVoltageSystem}</p></div>
                  <div><p className="text-gray-500">Capacity</p><p className="font-medium">{selectedEnquiry.systemCapacity || 'N/A'}</p></div>
                  <div><p className="text-gray-500">Rectifiers</p><p className="font-medium">{selectedEnquiry.numberOfRectifiers}</p></div>
                </div>
              </div>
...
export default DcPowerServices;
