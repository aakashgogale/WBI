...
const PowerTestingServices = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/powertestingservice-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/powertestingservice-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Power Testing Services</h1>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-600">Site Type</th>
                ...
                {loading ? (
                  ...
                  enquiries.map((enq) => (
                    <tr key={enq._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-sm text-gray-600">{enq.siteType}</td>
                      <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex items-center justify-between z-10">
              <h2 className="text-xl font-bold text-gray-800">Testing Service Details</h2>
            ...
            <div className="p-6 space-y-8">
              ...
              {/* Site Details */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Site & Capacity Details</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Site Type</p><p className="font-medium">{selectedEnquiry.siteType}</p></div>
                  <div><p className="text-gray-500">Total Capacity</p><p className="font-medium">{selectedEnquiry.totalPowerCapacity}</p></div>
                  <div><p className="text-gray-500">Locations</p><p className="font-medium">{selectedEnquiry.numberOfLocations}</p></div>
                  <div><p className="text-gray-500">Operational</p><p className="font-medium">{selectedEnquiry.isSiteOperational}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    <p className="text-gray-500 mb-1">Testing Required</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedEnquiry.testingRequired?.map((srv, i) => (
                        <span key={i} className="px-3 py-1 bg-[#E8F5F1] text-[#10AFA5] rounded-full text-xs font-medium">{srv}</span>
                      ))}
                    </div>
                  </div>
                  <div>
                    <p className="text-gray-500 mb-1">Equipment to Test</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedEnquiry.equipmentToTest?.map((srv, i) => (
                        <span key={i} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">{srv}</span>
                      ))}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    <div><p className="text-gray-500">Reason</p><p className="font-medium">{selectedEnquiry.reasonForTesting}</p></div>
...
export default PowerTestingServices;
