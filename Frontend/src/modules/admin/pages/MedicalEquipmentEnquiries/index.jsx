...
const MedicalEquipmentEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/medicaleq-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/medicaleq-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Engineer Assigned': return 'bg-yellow-100 text-yellow-800';
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Medical Equipment Enquiries</h1>
                ...
                {loading ? (
                  ...
                  enquiries.map((enq) => (
                    <tr key={enq._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-sm text-gray-600">{enq.equipmentNameBrand}</td>
                      <td className="p-4">
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Departments/Floors</p><p className="font-medium">{selectedEnquiry.departmentsCount}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Age</p><p className="font-medium">{selectedEnquiry.equipmentAge}</p></div>
                  <div><p className="text-gray-500">Warranty Status</p><p className="font-medium">{selectedEnquiry.warrantyStatus}</p></div>
                </div>
              </div>

              {/* Service Required */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Service Required</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div className="col-span-2 md:col-span-3">
                  ...
                  <div className="col-span-2 md:col-span-3"><p className="text-gray-500">Issue Description</p><p className="font-medium p-3 bg-gray-50 rounded-lg">{selectedEnquiry.issueDescription}</p></div>
                  <div><p className="text-gray-500">Error Codes</p><p className="font-medium">{selectedEnquiry.errorCodes || 'N/A'}</p></div>
                  <div><p className="text-gray-500">Is Critical?</p><p className="font-medium">{selectedEnquiry.isCritical}</p></div>
                </div>
              </div>
...
export default MedicalEquipmentEnquiries;
