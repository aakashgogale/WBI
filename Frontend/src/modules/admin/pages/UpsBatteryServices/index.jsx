...
const UpsBatteryServices = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/upsbatteryservice-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/upsbatteryservice-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">UPS Battery Services</h1>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-600">UPS Brand/Cap</th>
                ...
                {loading ? (
                  ...
                  enquiries.map((enq) => (
                    <tr key={enq._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-sm text-gray-600">{enq.upsBrand} ({enq.upsCapacity})</td>
                      <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex items-center justify-between z-10">
              <h2 className="text-xl font-bold text-gray-800">UPS Battery Service Details</h2>
            ...
            <div className="p-6 space-y-8">
              ...
              {/* UPS Details */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">UPS Details</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Brand</p><p className="font-medium">{selectedEnquiry.upsBrand}</p></div>
                  <div><p className="text-gray-500">Capacity</p><p className="font-medium">{selectedEnquiry.upsCapacity}</p></div>
                  <div><p className="text-gray-500">Age</p><p className="font-medium">{selectedEnquiry.upsAge}</p></div>
                  <div><p className="text-gray-500">Type</p><p className="font-medium">{selectedEnquiry.upsType}</p></div>
                  <div><p className="text-gray-500">Connected Equipment</p><p className="font-medium">{selectedEnquiry.connectedEquipment}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Error Description</p><p className="font-medium p-3 bg-gray-50 rounded-lg">{selectedEnquiry.errorDescription}</p></div>
                </div>
              </div>
...
export default UpsBatteryServices;
