...
const ElectricalSafetyTestEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/safetytest-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/safetytest-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Electrical Safety Test Enquiries</h1>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-600">Equipment</th>
                ...
                {loading ? (
                  ...
                  enquiries.map((enq) => (
                    <tr key={enq._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-sm text-gray-600">{enq.equipmentNameBrand} ({enq.numberOfUnits} units)</td>
                      <td className="p-4">
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Is Accredited (NABH/NABL)</p><p className="font-medium">{selectedEnquiry.isAccredited}</p></div>
                  <div className="col-span-2"><p className="text-gray-500">Testing Required For</p><p className="font-medium">{selectedEnquiry.testingRequiredFor}</p></div>
                </div>
              </div>

              {/* Equipment Details */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Equipment to be Tested</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Name & Brand</p><p className="font-medium">{selectedEnquiry.equipmentNameBrand}</p></div>
                  <div><p className="text-gray-500">Number of Units</p><p className="font-medium">{selectedEnquiry.numberOfUnits}</p></div>
                  <div><p className="text-gray-500">Equipment Age/Last Tested</p><p className="font-medium">{selectedEnquiry.equipmentAgeLastTested || 'N/A'}</p></div>
                </div>
              </div>

              {/* Testing Requirements */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Testing Requirements</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div className="col-span-2 md:col-span-3">
                    <p className="text-gray-500 mb-1">Type of Tests Required</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedEnquiry.typeOfTestsRequired?.map((srv, i) => (
                        <span key={i} className="px-3 py-1 bg-[#E8F5F1] text-[#10AFA5] rounded-full text-xs font-medium">{srv}</span>
                      ))}
                    </div>
                  </div>
                  <div><p className="text-gray-500">Applicable Standard</p><p className="font-medium">{selectedEnquiry.applicableStandard}</p></div>
                  <div><p className="text-gray-500">Need Test Certificates?</p><p className="font-medium">{selectedEnquiry.needTestCertificates}</p></div>
                  <div><p className="text-gray-500">Need NABL Traceable?</p><p className="font-medium">{selectedEnquiry.needNablTraceable}</p></div>
                  <div><p className="text-gray-500">Need Pass/Fail Status?</p><p className="font-medium">{selectedEnquiry.needPassFailStatus}</p></div>
                  <div><p className="text-gray-500">Report Format</p><p className="font-medium">{selectedEnquiry.reportFormat}</p></div>
                  <div><p className="text-gray-500">For NABH Submission?</p><p className="font-medium">{selectedEnquiry.isReportForNabh}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Preferred Date</p><p className="font-medium">{selectedEnquiry.preferredVisitDate ? new Date(selectedEnquiry.preferredVisitDate).toLocaleDateString() : 'N/A'}</p></div>
                  <div><p className="text-gray-500">Preferred Time</p><p className="font-medium">{selectedEnquiry.preferredTimeSlot}</p></div>
                  <div><p className="text-gray-500">Working Hours Testing?</p><p className="font-medium">{selectedEnquiry.workingHoursTesting}</p></div>
                  <div><p className="text-gray-500">Urgency</p><p className="font-medium">{selectedEnquiry.urgency}</p></div>
                  <div><p className="text-gray-500">Department</p><p className="font-medium">{selectedEnquiry.departmentLocation}</p></div>
                  <div><p className="text-gray-500">Contact Person</p><p className="font-medium">{selectedEnquiry.siteContactName} ({selectedEnquiry.siteContactNumber})</p></div>
                  <div><p className="text-gray-500">Budget Range</p><p className="font-medium">{selectedEnquiry.budgetRange}</p></div>
                  <div><p className="text-gray-500">Interested in Annual Contract?</p><p className="font-medium">{selectedEnquiry.interestedInAnnualContract}</p></div>
                  <div><p className="text-gray-500">Interested in AMC?</p><p className="font-medium">{selectedEnquiry.interestedInAmc}</p></div>
...
export default ElectricalSafetyTestEnquiries;
