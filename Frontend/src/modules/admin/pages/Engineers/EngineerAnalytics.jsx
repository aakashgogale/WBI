...
const EngineerAnalytics = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await adminReportService.getEngineerReport();
      if (res.success) {
        setData(res.data);
      }
    } catch (error) {
      console.error('Engineer analytics error:', error);
      toast.error('Failed to load engineer analytics');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <CardShell className="bg-gradient-to-br from-blue-500 to-blue-600 text-white border-none p-3.5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-100 text-[10px] font-medium uppercase tracking-wider">Total Active Engineers</p>
        ...
        <CardShell className="bg-gradient-to-br from-green-500 to-green-600 text-white border-none p-3.5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-xl font-bold mt-0.5">
                {data?.topEngineers?.reduce((acc, curr) => acc + curr.completedJobs, 0) || 0}
              </h3>
            </div>
            <div className="bg-white/20 p-2 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Top Engineers Chart */}
        <CardShell className="bg-white p-3.5" title="Top Performing Engineers" icon={FiActivity}>
        ...
        <CardShell className="bg-white p-3.5" title="Engineer Status Distribution" icon={FiUsers}>
          <div className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
