...
const HcAmcEnquiries = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const res = await api.get('/admin/hcamc-enquiries');
      if (res.data.success) {
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const res = await api.put(`/admin/hcamc-enquiries/${id}/status`, { status: newStatus });
      if (res.data.success) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'Contract Signed': return 'bg-green-100 text-green-800';
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Annual Maintenance Contract Enquiries</h1>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-600">AMC Type</th>
                ...
                {loading ? (
                  ...
                  enquiries.map((enq) => (
                    <tr key={enq._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-sm text-gray-600">{enq.amcTypeRequired}</td>
                      <td className="p-4">
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Alternate Contact</p><p className="font-medium">{selectedEnquiry.alternateContact || 'N/A'}</p></div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Branches Count</p><p className="font-medium">{selectedEnquiry.branchesCount}</p></div>
                  <div><p className="text-gray-500">Is Accredited?</p><p className="font-medium">{selectedEnquiry.isAccredited}</p></div>
                  <div><p className="text-gray-500">Total Equipment Count</p><p className="font-medium">{selectedEnquiry.totalEquipmentCount}</p></div>
                </div>
              </div>

              {/* Equipment Details */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Equipment to be Covered Under AMC</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div><p className="text-gray-500">Inventory Available?</p><p className="font-medium">{selectedEnquiry.inventoryAvailable}</p></div>
                  <div><p className="text-gray-500">Equipment Not Working?</p><p className="font-medium">{selectedEnquiry.equipmentNotWorking}</p></div>
                </div>
              </div>

              {/* AMC Type & Coverage */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">AMC Type & Coverage</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div className="col-span-2 md:col-span-3">
                    <p className="text-gray-500 mb-1">Coverage Required</p>
                    <div className="flex flex-wrap gap-2">
                      {selectedEnquiry.coverageRequired?.map((srv, i) => (
                        <span key={i} className="px-3 py-1 bg-[#E8F5F1] text-[#10AFA5] rounded-full text-xs font-medium">{srv}</span>
                      ))}
                    </div>
                  </div>
                  <div><p className="text-gray-500">AMC Type Required</p><p className="font-medium">{selectedEnquiry.amcTypeRequired}</p></div>
                  <div><p className="text-gray-500">Expected Response Time</p><p className="font-medium">{selectedEnquiry.responseTimeExpected}</p></div>
                  <div><p className="text-gray-500">PM Visits Per Year</p><p className="font-medium">{selectedEnquiry.pmVisitsPerYear}</p></div>
                </div>
              </div>

              {/* Current AMC Status */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Current AMC Status</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Have Existing AMC?</p><p className="font-medium">{selectedEnquiry.haveExistingAmc}</p></div>
                  <div><p className="text-gray-500">Current AMC Provider</p><p className="font-medium">{selectedEnquiry.currentAmcProvider || 'N/A'}</p></div>
                  <div><p className="text-gray-500">AMC Expiry Date</p><p className="font-medium">{selectedEnquiry.amcExpiryDate || 'N/A'}</p></div>
                  <div><p className="text-gray-500">Why Switching?</p><p className="font-medium">{selectedEnquiry.whySwitching || 'N/A'}</p></div>
                  <div><p className="text-gray-500">Maintained by OEM?</p><p className="font-medium">{selectedEnquiry.maintainedByOem}</p></div>
                  <div><p className="text-gray-500">Any Pending Breakdowns?</p><p className="font-medium">{selectedEnquiry.pendingBreakdowns}</p></div>
                  <div><p className="text-gray-500">Breakdown Calls Last Year</p><p className="font-medium">{selectedEnquiry.breakdownCallsLastYear}</p></div>
                </div>
              </div>

              {/* Compliance & Documentation */}
              <div>
                <h3 className="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Compliance & Documentation Requirements</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Need NABH Records?</p><p className="font-medium">{selectedEnquiry.needNabhRecords}</p></div>
                  <div><p className="text-gray-500">Need Calibration Certificates?</p><p className="font-medium">{selectedEnquiry.needCalibrationCertificates}</p></div>
                  <div><p className="text-gray-500">Need Safety Test Certificates?</p><p className="font-medium">{selectedEnquiry.needSafetyTestCertificates}</p></div>
                  ...
                  <div><p className="text-gray-500">Need MIS Reports?</p><p className="font-medium">{selectedEnquiry.needMisReports}</p></div>
                  <div><p className="text-gray-500">Need Dedicated Engineer?</p><p className="font-medium">{selectedEnquiry.needDedicatedEngineer}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div><p className="text-gray-500">Preferred Start Date</p><p className="font-medium">{selectedEnquiry.preferredAmcStartDate ? new Date(selectedEnquiry.preferredAmcStartDate).toLocaleDateString() : 'N/A'}</p></div>
                  <div><p className="text-gray-500">AMC Duration Required</p><p className="font-medium">{selectedEnquiry.amcDurationRequired}</p></div>
                  <div><p className="text-gray-500">Pre-AMC Inspection Needed?</p><p className="font-medium">{selectedEnquiry.preAmcInspectionNeeded}</p></div>
                  <div><p className="text-gray-500">Annual Budget</p><p className="font-medium">{selectedEnquiry.annualAmcBudget}</p></div>
                  <div><p className="text-gray-500">Payment Preference</p><p className="font-medium">{selectedEnquiry.paymentPreference}</p></div>
