...
import { 
  ...
  FiMapPin, FiSmartphone, FiTool, FiClock, FiFileText
} from 'react-icons/fi';
...
const PosService = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Section 2
    posType: '', posBrand: '', posModelNumber: '', terminalId: '', numberOfMachines: '', linkedBank: '',
    // Section 3
    serviceRequired: '', posStatus: '', errorShown: '', issueDescription: '', issueStartTime: '',
    // Section 4
  ...
  // Dropdowns
  const posTypes = ['Countertop POS Terminal', 'Mobile / Wireless POS', 'Android POS', 'mPOS (mini POS)', 'Kiosk POS', 'Self-Service POS', 'Other'];
  const brands = ['Verifone', 'Ingenico', 'PAX', 'Newland', 'Mswipe', 'Pine Labs', 'Other (specify)'];
  const services = ['Breakdown / Not working', 'Card swipe / tap not reading', 'Receipt printer not printing', 'Connectivity / network issue', 'Screen damage / not responding', 'Battery issue (wireless POS)', 'Software / app update', 'New POS installation', 'POS relocation', 'Preventive maintenance', 'AMC'];
  ...
  const urgencies = ['Emergency — transactions failing', 'Same day', 'Within 24 hours', 'Within 2–3 days', 'Flexible'];
  const budgets = ['To be discussed', 'Below ₹2,000', '₹2,000–₹5,000', 'Above ₹5,000'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/posservice-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our POS service team will contact you within 24 hours.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">POS Service</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiSmartphone className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">POS Terminals</h2>
            <p className="text-gray-600 text-sm">Swift repair & maintenance for all card swipe machines.</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#E8F5F1] p-6 border-l-4 border-l-[#10AFA5]">
            ...
            <div className="space-y-4">
              ...
              <input required type="text" name="organizationName" value={formData.organizationName} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Business / Bank Name *" />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiSmartphone className="text-[#10AFA5]" /> 2. POS Machine Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="posType" value={formData.posType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">POS Type *</option>
                {posTypes.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select required name="posBrand" value={formData.posBrand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">POS Brand *</option>
                {brands.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <input required type="text" name="terminalId" value={formData.terminalId} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Terminal ID / TID *" />
              <input type="text" name="posModelNumber" value={formData.posModelNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Model Number (optional)" />
              <input required type="number" name="numberOfMachines" value={formData.numberOfMachines} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of POS machines *" />
              <input required type="text" name="linkedBank" value={formData.linkedBank} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Bank / Gateway Linked *" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <select required name="posStatus" value={formData.posStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  <option value="">Current POS Status *</option>
              ...
              <input type="text" name="errorShown" value={formData.errorShown} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Error shown (if any)" />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Error Screenshots, Machine Photos, or Warranty docs.</p>
