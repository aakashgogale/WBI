...
import { 
  ...
  FiMapPin, FiMaximize, FiTool, FiClock, FiFileText, FiBatteryCharging, FiAlertTriangle
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';
import BottomNav from '../../components/layout/BottomNav';

const UpsBatteryService = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    upsBrand: '', upsModelNumber: '', upsCapacity: '', upsType: '', upsAge: '', numberOfUpsUnits: '', connectedEquipment: '',
    batteryBrand: '', batteryType: '', numberOfBatteries: '', batteryAge: '', lastBatteryReplacement: '',
    serviceRequired: [], currentStatus: '', errorDescription: '', issueStartTime: '',
    affectedEquipment: '', isBusinessAffected: '', isDataAtRisk: '',
    preferredVisitDate: '', preferredTimeSlot: '', urgency: '', siteContactName: '', siteContactNumber: '', upsRoomAccess: '',
    budgetRange: '', needHealthReport: '', interestedInAmc: '', source: '', additionalNotes: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/upsbatteryservice-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our UPS specialist will contact you shortly to restore your backup.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">UPS & Battery Service</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiBatteryCharging className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">UPS Systems & Batteries</h2>
            <p className="text-gray-600 text-sm">Critical power backup restoration and UPS preventative care.</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4 -mt-4 relative z-10">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiMaximize className="text-[#10AFA5]" /> 2. UPS Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="upsBrand" value={formData.upsBrand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">UPS Brand *</option>
                {['APC / Schneider', 'Emerson / Vertiv', 'Eaton', 'Luminous', 'Su-Kam', 'Microtek', 'Delta', 'Other'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <input type="text" name="upsModelNumber" value={formData.upsModelNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="UPS Model Number" />
              <select required name="upsCapacity" value={formData.upsCapacity} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">UPS Capacity *</option>
                {['Below 1 KVA', '1 – 3 KVA', '3 – 10 KVA', '10 – 40 KVA', '40 – 100 KVA', 'Above 100 KVA'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select required name="upsType" value={formData.upsType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">UPS Type *</option>
                {['Offline / Standby UPS', 'Line Interactive UPS', 'Online Double Conversion UPS', 'Modular UPS', 'Not sure'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select required name="upsAge" value={formData.upsAge} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">UPS Age *</option>
                {['Less than 1 yr', '1–3 yrs', '3–5 yrs', 'More than 5 yrs'].map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              <input required type="number" name="numberOfUpsUnits" value={formData.numberOfUpsUnits} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of UPS units *" />
              <select required name="connectedEquipment" value={formData.connectedEquipment} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Equipment connected to UPS *</option>
                {['Servers', 'Computers', 'ATM', 'Medical equipment', 'Other'].map(e => <option key={e} value={e}>{e}</option>)}
              </select>
            </div>
          </div>

          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiBatteryCharging className="text-[#10AFA5]" /> 3. Internal Battery Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="batteryBrand" value={formData.batteryBrand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Brand inside UPS *</option>
                {['Exide', 'Amaron', 'APC', 'Luminous', 'Other', 'Not sure'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="batteryType" value={formData.batteryType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Type *</option>
                {['VRLA SMF', 'Lithium', 'Other', 'Not sure'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <input required type="text" name="numberOfBatteries" value={formData.numberOfBatteries} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of batteries (or 'Not sure') *" />
              <select required name="batteryAge" value={formData.batteryAge} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Age *</option>
                {['Less than 1 yr', '1–3 yrs', '3–5 yrs', 'More than 5 yrs', 'Not sure'].map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              <input type="text" name="lastBatteryReplacement" value={formData.lastBatteryReplacement} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Last battery replacement date (or 'Don't remember')" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['UPS battery replacement', 'UPS not giving backup', 'UPS showing battery fault alarm', 'UPS shutdown / not turning on', 'UPS beeping continuously', 'Battery swollen / leaking', 'Battery health check & load test', 'Full UPS preventive maintenance', 'New UPS + battery installation', 'AMC'].map(s => (
            ...
            <div className="space-y-4">
              <select required name="currentStatus" value={formData.currentStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Current UPS Status *</option>
                {['UPS completely down', 'No backup — switches off on power cut', 'Very low backup time', 'Battery fault alarm showing', 'Beeping / alarming', 'Overheating', 'Working — routine check'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <textarea required name="errorDescription" value={formData.errorDescription} onChange={handleInputChange} rows="3" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Error / alarm description *"></textarea>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiAlertTriangle className="text-[#10AFA5]" /> 5. Impact Assessment</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="affectedEquipment" value={formData.affectedEquipment} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">What equipment is affected? *</option>
                {['Servers', 'Computers', 'ATM', 'CCTV', 'Medical', 'Other'].map(e => <option key={e} value={e}>{e}</option>)}
              </select>
              <select required name="isBusinessAffected" value={formData.isBusinessAffected} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Is business affected? *</option>
                <option value="Yes — critically">Yes — critically</option>
                <option value="Partially">Partially</option>
                <option value="No">No</option>
              </select>
              <select required name="isDataAtRisk" value={formData.isDataAtRisk} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Is data at risk? *</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select required name="urgency" value={formData.urgency} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-[#E8F5F1] text-[#10AFA5] border border-[#10AFA5] font-bold rounded-xl outline-none">
                ...
                {['Emergency', 'Same day', 'Within 24 hrs', 'Within 2–3 days', 'Flexible'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
              ...
              <select required name="upsRoomAccess" value={formData.upsRoomAccess} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">UPS room access available? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="budgetRange" value={formData.budgetRange} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                ...
                {['Below ₹3,000', '₹3,000–₹10,000', '₹10,000–₹30,000', '₹30,000–₹1,00,000', 'Above ₹1,00,000', 'TBD'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="needHealthReport" value={formData.needHealthReport} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need UPS health report? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload UPS Errors, AMC docs, or Previous reports.</p>
...
export default UpsBatteryService;
