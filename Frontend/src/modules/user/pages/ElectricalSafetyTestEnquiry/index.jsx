...
import { 
  ...
  FiMapPin, FiMaximize, FiTool, FiClock, FiFileText, FiShield
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';
import BottomNav from '../../components/layout/BottomNav';

const ElectricalSafetyTestEnquiry = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    facilityType: '', facilitySize: '', isAccredited: '', testingRequiredFor: '',
    equipmentCategory: [], equipmentNameBrand: '', numberOfUnits: '', equipmentAgeLastTested: '',
    typeOfTestsRequired: [], applicableStandard: '', needTestCertificates: '', needNablTraceable: '', needPassFailStatus: '', reportFormat: '', isReportForNabh: '',
    preferredVisitDate: '', preferredTimeSlot: '', workingHoursTesting: '', departmentLocation: '', siteContactName: '', siteContactNumber: '', urgency: '',
    budgetRange: '', interestedInAnnualContract: '', interestedInAmc: '', howDidYouHear: '', additionalNotes: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/safetytest-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">Electrical Safety Test</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiShield className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">Electrical Safety Test</h2>
            <p className="text-gray-600 text-sm">Patient and equipment electrical safety testing.</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select required name="testingRequiredFor" value={formData.testingRequiredFor} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Testing required for *</option>
                {['Routine periodic check', 'Pre-accreditation / audit preparation', 'Post-repair verification', 'New equipment acceptance test', 'Incident investigation', 'Other'].map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['Class I Equipment (Earthed)', 'Class II Equipment (Double Insulated)', 'Internally Powered Equipment', 'Defibrillators', 'Patient Monitors', 'Electrosurgical Units (ESU)', 'Infusion Pumps', 'Dental Chairs', 'Physiotherapy Equipment', 'Other'].map(s => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input type="text" name="equipmentAgeLastTested" value={formData.equipmentAgeLastTested} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Equipment Age / Last Tested Date" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm font-medium text-gray-700 mb-2">Type of Tests Required *</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['Earth Leakage Current Test', 'Enclosure Leakage Current Test', 'Patient Leakage Current Test', 'Patient Auxiliary Current Test', 'Insulation Resistance Test', 'Earth Bonding / Ground Wire Resistance', 'Mains Voltage & Current Consumption', 'Defibrillator Protection Test', 'Full standard compliance test', 'Not sure — need expert assessment'].map(s => (
                <label key={s} className="flex items-start gap-2 p-2 border rounded-lg cursor-pointer hover:bg-gray-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="applicableStandard" value={formData.applicableStandard} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Applicable Standard *</option>
                {['IEC 60601-1', 'IEC 62353', 'Not sure', 'Other'].map(o => <option key={o} value={o}>{o}</option>)}
              </select>
              <select required name="needTestCertificates" value={formData.needTestCertificates} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need test certificates per equipment? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <select required name="needNablTraceable" value={formData.needNablTraceable} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need NABL traceable testing? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
                <option value="Not sure">Not sure</option>
              </select>
              <select required name="needPassFailStatus" value={formData.needPassFailStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
              ...
              <select required name="isReportForNabh" value={formData.isReportForNabh} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Is report needed for NABH submission? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="budgetRange" value={formData.budgetRange} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                ...
                {['Below ₹2,999', '₹2,999 – ₹10,000', '₹10,000 – ₹25,000', '₹25,000 – ₹50,000', 'Above ₹50,000', 'To be discussed'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="interestedInAnnualContract" value={formData.interestedInAnnualContract} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Interested in annual contract? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <select required name="interestedInAmc" value={formData.interestedInAmc} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Interested in Equipment AMC? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Equipment List, Previous Reports, or NABH Checklist.</p>
...
export default ElectricalSafetyTestEnquiry;
