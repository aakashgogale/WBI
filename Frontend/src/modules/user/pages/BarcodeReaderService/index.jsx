...
import { 
  ...
  FiMapPin, FiMaximize, FiTool, FiClock, FiFileText
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';
import BottomNav from '../../components/layout/BottomNav';

const BarcodeReaderService = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Section 2
    readerType: '', brand: '', modelNumber: '', numberOfDevices: '', usedFor: '',
    // Section 3
    serviceRequired: '', currentStatus: '', issueDescription: '', issueStartTime: '', connectedTo: '', softwareName: '',
    // Section 4
    preferredVisitDate: '', preferredTimeSlot: '', urgency: '', siteContactName: '', siteContactNumber: '',
    // Section 5
    needServiceReport: '', sparePartsReplacement: '', interestedInAmc: '', budgetRange: '', source: '', additionalNotes: ''
  });
  ...
  // Dropdowns
  const readerTypes = ['Handheld Barcode Scanner', 'Fixed Mount Scanner', 'Wireless / Bluetooth Scanner', '2D Barcode / QR Code Scanner', 'Barcode Printer + Scanner combo', 'Industrial Barcode Reader', 'Other'];
  const brands = ['Honeywell', 'Zebra / Motorola', 'Datalogic', 'Symbol', 'Opticon', 'TVS', 'Other (specify)'];
  const usages = ['Retail billing / POS', 'Warehouse / inventory management', 'Bank / document scanning', 'Healthcare / patient tracking', 'Asset tracking', 'Manufacturing / production', 'Other'];
  const services = ['Breakdown / not scanning', 'Scanning accuracy issue', 'Connectivity issue (USB / Bluetooth / WiFi)', 'Physical damage (dropped / cracked)', 'Battery issue (wireless scanner)', 'Software / driver issue', 'New device installation & setup', 'Preventive maintenance', 'AMC'];
  const statuses = ['Working', 'Not working', 'Intermittent'];
  const startTimes = ['Just now', 'Today', 'Yesterday', '2–3 days', 'More than a week'];
  const connections = ['POS software', 'ERP', 'Custom software', 'Standalone'];
  const timeSlots = ['Morning 9–12', 'Afternoon 12–4', 'Evening 4–7'];
  const urgencies = ['Emergency — operations stopped', 'Same day', 'Within 24 hours', 'Within 2–3 days', 'Flexible'];
  const budgets = ['To be discussed', 'Below ₹2,000', '₹2,000–₹8,000', 'Above ₹8,000'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/barcodereaderservice-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our hardware team will contact you within 24 hours.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">Barcode Reader Service</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">Barcode Scanners</h2>
            <p className="text-gray-600 text-sm">Fast repairs for POS, warehouse, and retail barcode devices.</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#E8F5F1] p-6 border-l-4 border-l-[#10AFA5]">
            ...
            <div className="space-y-4">
              ...
              <input required type="text" name="siteAddress" value={formData.siteAddress} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Site Address *" />
            </div>
          </div>

          {/* Section 2: Barcode Reader Details */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiMaximize className="text-[#10AFA5]" /> 2. Barcode Reader Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="readerType" value={formData.readerType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Barcode Reader Type *</option>
                {readerTypes.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select required name="brand" value={formData.brand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Brand *</option>
                {brands.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <input type="text" name="modelNumber" value={formData.modelNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Model Number (optional)" />
              <input required type="number" name="numberOfDevices" value={formData.numberOfDevices} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of devices *" />
              <select required name="usedFor" value={formData.usedFor} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Used for (Application) *</option>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <select required name="currentStatus" value={formData.currentStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  <option value="">Current Device Status *</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <select required name="connectedTo" value={formData.connectedTo} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  <option value="">Device connected to *</option>
                  {connections.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <input type="text" name="softwareName" value={formData.softwareName} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Software / Application name (optional)" />
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input required type="text" name="siteContactName" value={formData.siteContactName} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Site Contact Person *" />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiFileText className="text-[#10AFA5]" /> 5. Additional Info</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select required name="sparePartsReplacement" value={formData.sparePartsReplacement} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Error Screenshots, Device Photos, or Warranty docs.</p>
