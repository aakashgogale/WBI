...
import { 
  ...
  FiMapPin, FiMaximize, FiTool, FiClock, FiFileText, FiBattery
} from 'react-icons/fi';
...
const BatteryService = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    siteType: '', numberOfBatteryBanks: '', isSiteCritical: '',
    batteryType: '', batteryBrand: '', batteryCapacity: '', batteryVoltage: '', numberOfUnits: '', batteryAge: '', connectedTo: '',
    serviceRequired: [], currentStatus: '', issueDescription: '', issueStartTime: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/batteryservice-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our battery expert will contact you shortly.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">Battery Service</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiBattery className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">Industrial Battery Service</h2>
            <p className="text-gray-600 text-sm">Testing, replacement, and health checks for large battery banks.</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4 -mt-4 relative z-10">
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-100">
                <select required name="siteType" value={formData.siteType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  ...
                  {['Office', 'Hospital', 'Bank', 'Data Center', 'Telecom Tower', 'Factory', 'Residential', 'Other'].map(o => <option key={o} value={o}>{o}</option>)}
                </select>
                <input required type="number" name="numberOfBatteryBanks" value={formData.numberOfBatteryBanks} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Banks count *" />
                <select required name="isSiteCritical" value={formData.isSiteCritical} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  <option value="">Is site critical? *</option>
                  <option value="Yes">Yes (24x7)</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiBattery className="text-[#10AFA5]" /> 2. Battery Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="batteryType" value={formData.batteryType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                ...
                {['VRLA / SMF Battery (Sealed Maintenance Free)', 'Tubular / Flooded Lead Acid Battery', 'Lithium-Ion Battery', 'Nickel Cadmium (NiCd)', 'Gel Battery', 'Not sure'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <select required name="batteryBrand" value={formData.batteryBrand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Brand *</option>
                {['Exide', 'Amaron', 'Su-Kam', 'Luminous', 'HBL', 'Fiamm', 'Leoch', 'Other'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="batteryCapacity" value={formData.batteryCapacity} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Capacity (Ah) *</option>
                {['Below 42 Ah', '42 – 100 Ah', '100 – 200 Ah', '200 – 500 Ah', 'Above 500 Ah', 'Not sure'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select required name="batteryVoltage" value={formData.batteryVoltage} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery Voltage *</option>
                {['6V', '12V', '24V', '48V', 'Other'].map(v => <option key={v} value={v}>{v}</option>)}
              </select>
              <input required type="number" name="numberOfUnits" value={formData.numberOfUnits} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of Units / Cells *" />
              ...
              <select required name="connectedTo" value={formData.connectedTo} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Battery connected to *</option>
                {['UPS', 'DG', 'Solar', 'Inverter', 'Telecom', 'Other'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['Breakdown / Battery not charging', 'Battery replacement', 'Battery health check / load test', 'Battery bank cleaning & maintenance', 'Battery terminal & connection service', 'Electrolyte top-up (flooded batteries)', 'Battery capacity testing', 'New battery bank installation', 'AMC'].map(s => (
            ...
            <div className="space-y-4">
              <select required name="currentStatus" value={formData.currentStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Current Battery Status *</option>
                {['Not charging at all', 'Discharging too fast', 'Swollen / physically damaged', 'Overheating', 'Low backup time', 'Showing error on charger / UPS', 'Working — routine maintenance'].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiClock className="text-[#10AFA5]" /> 4. Visit & Urgency</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select required name="needHealthReport" value={formData.needHealthReport} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need health report? *</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Battery Photos, Errors, or Reports.</p>
