...
const HcPreventiveMaintenanceEnquiry = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    facilityType: '', facilitySize: '', departmentsCount: '', locationsCount: '', isAccredited: '',
    equipmentCategory: [], totalUnits: '', equipmentBrands: '', equipmentAgeRange: '', equipmentListAvailable: '',
    isPmCurrentlyDone: '', whoIsDoingPm: '', lastPmDoneDate: '', anyExistingAmc: '', providerExpiryDate: '', currentIssues: '', breakdownCallsPerMonth: '',
    maintenanceActivitiesNeeded: [], maintenanceFrequency: '', needPmReportAfterVisit: '', needEquipmentHistory: '',
    preferredFirstVisitDate: '', preferredTimeSlot: '', workingHoursMaintenance: '', siteContactName: '', siteContactNumber: '', accessRestriction: '', urgency: '',
    budgetPerVisit: '', interestedInFullAmc: '', howDidYouHear: '', additionalNotes: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.maintenanceActivitiesNeeded.length === 0) {
      toast.error('Please select at least one maintenance activity');
      return;
    }
    ...
    try {
      ...
      if (!finalData.preferredFirstVisitDate) delete finalData.preferredFirstVisitDate;

      const res = await api.post('/public/hcpm-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our biomedical team will contact you shortly to schedule PM.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiTool className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">Preventive Maintenance</h2>
            <p className="text-gray-600 text-sm">Regular upkeep & maintenance of medical equipment.</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input required type="text" name="locationsCount" value={formData.locationsCount} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of Locations / Branches *" />
              <select required name="isAccredited" value={formData.isAccredited} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['All Medical Equipment (Complete Hospital)', 'ICU & Life Support Equipment', 'OT / Surgical Equipment', 'Radiology / Imaging Equipment', 'Laboratory Equipment', 'CSSD / Sterilization Equipment', 'Patient Ward Equipment', 'Other'].map(s => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input required type="number" name="totalUnits" value={formData.totalUnits} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Total Number of Units *" />
              ...
              <select required name="equipmentListAvailable" value={formData.equipmentListAvailable} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Is complete equipment list available? *</option>
                <option value="Yes">Yes</option>
          ...
          {/* Section 4 */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiClock className="text-[#10AFA5]" /> 4. Maintenance History</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="isPmCurrentlyDone" value={formData.isPmCurrentlyDone} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Is PM currently being done? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <input type="text" name="whoIsDoingPm" value={formData.whoIsDoingPm} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Who is doing it? (In-house/Agency)" />
              <input type="text" name="lastPmDoneDate" value={formData.lastPmDoneDate} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Last PM done date" />
              <select required name="anyExistingAmc" value={formData.anyExistingAmc} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Any existing AMC contract? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <input type="text" name="providerExpiryDate" value={formData.providerExpiryDate} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Provider & Expiry Date (if yes)" />
              <input type="text" name="breakdownCallsPerMonth" value={formData.breakdownCallsPerMonth} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Avg breakdown calls per month" />
            </div>
            <textarea name="currentIssues" value={formData.currentIssues} onChange={handleInputChange} rows="2" className="w-full mt-4 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Current issues / frequent complaints"></textarea>
          </div>

          {/* Section 5 */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiTool className="text-[#10AFA5]" /> 5. Maintenance Requirements</h3>
            <p className="text-sm font-medium text-gray-700 mb-2">Maintenance Activities Needed *</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['Visual Inspection & Cleaning', 'Functional Testing', 'Electrical Safety Testing', 'Calibration & Verification', 'Filter / Consumable Check', 'Lubrication & Adjustment', 'Software updates & Logs check', 'Asset Tagging & Inventory', 'Other'].map(s => (
                <label key={s} className="flex items-start gap-2 p-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" value={s} checked={formData.maintenanceActivitiesNeeded.includes(s)} onChange={(e) => handleCheckboxChange(e, 'maintenanceActivitiesNeeded')} className="mt-1 text-[#10AFA5] focus:ring-[#10AFA5] rounded" />
                  <span className="text-sm text-gray-700">{s}</span>
                </label>
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="maintenanceFrequency" value={formData.maintenanceFrequency} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Maintenance Frequency *</option>
                {['One-time comprehensive PM', 'Monthly', 'Quarterly', 'Bi-annually', 'Annually'].map(o => <option key={o} value={o}>{o}</option>)}
              </select>
              <select required name="needPmReportAfterVisit" value={formData.needPmReportAfterVisit} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need PM report after visit? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <select required name="needEquipmentHistory" value={formData.needEquipmentHistory} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need equipment-wise history & asset tracking? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
            </div>
          ...
          {/* Section 6 */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiClock className="text-[#10AFA5]" /> 6. Visit & Timeline</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block px-1">Preferred First Visit Date *</label>
                <input required type="date" name="preferredFirstVisitDate" value={formData.preferredFirstVisitDate} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" />
              </div>
              <div>
              ...
              <select required name="workingHoursMaintenance" value={formData.workingHoursMaintenance} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Maintenance during working hours? *</option>
                <option value="Yes">Yes</option>
              ...
              <select required name="urgency" value={formData.urgency} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-[#E8F5F1] text-[#10AFA5] border border-[#10AFA5] font-bold rounded-xl outline-none">
                ...
                {['Immediate', 'Within 1 week', 'Within 2 weeks', 'Next month'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="budgetPerVisit" value={formData.budgetPerVisit} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Budget Per Visit *</option>
                {['Below ₹4,999', '₹4,999 – ₹15,000', '₹15,000 – ₹50,000', 'Above ₹50,000', 'To be discussed'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="interestedInFullAmc" value={formData.interestedInFullAmc} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Interested in Full AMC? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
                <option value="Need quotes for both PM and AMC">Need quotes for both</option>
              </select>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Equipment List, Existing AMC Contract, Previous Reports.</p>
...
export default HcPreventiveMaintenanceEnquiry;
