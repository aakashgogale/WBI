...
import { 
  ...
  FiMapPin, FiMaximize, FiTool, FiClock, FiFileText, FiActivity, FiTarget
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';
import BottomNav from '../../components/layout/BottomNav';

const PowerTestingService = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    siteType: '', totalPowerCapacity: '', numberOfFloors: '', numberOfLocations: '', isSiteOperational: '',
    testingRequired: [], equipmentToTest: [],
    reasonForTesting: '', needDetailedReport: '', reportFormat: '', neededForAuthority: '', needRecommendations: '',
    preferredVisitDate: '', preferredTimeSlot: '', canTestDuringWorkingHours: '', siteContactName: '', siteContactNumber: '', panelAccessAvailable: '', safetyRestrictions: '', projectUrgency: '',
    budgetRange: '', interestedInAmcAfter: '', source: '', additionalNotes: ''
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formData.testingRequired.length === 0 || formData.equipmentToTest.length === 0) {
      toast.error('Please select at least one testing type and one equipment type');
      return;
    }
    ...
    try {
      ...
      const res = await api.post('/public/powertestingservice-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our testing/auditing team will contact you shortly to plan the visit.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">Power System Testing</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiTarget className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">Testing & Audits</h2>
            <p className="text-gray-600 text-sm">Load testing, PQ analysis, thermography, and safety compliance.</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4 -mt-4 relative z-10">
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                <select required name="siteType" value={formData.siteType} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  ...
                  {['Office', 'Hospital', 'Bank', 'Data Center', 'Factory', 'Telecom', 'Government', 'Other'].map(o => <option key={o} value={o}>{o}</option>)}
                </select>
                <select required name="totalPowerCapacity" value={formData.totalPowerCapacity} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                  <option value="">Total Power Capacity *</option>
                  {['Below 10 KW', '10–50 KW', '50–200 KW', '200–500 KW', 'Above 500 KW', 'Not sure'].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <input type="number" name="numberOfFloors" value={formData.numberOfFloors} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of Floors" />
                <input required type="number" name="numberOfLocations" value={formData.numberOfLocations} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of Locations / Panels *" />
                <select required name="isSiteOperational" value={formData.isSiteOperational} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiTool className="text-[#10AFA5]" /> 2. Testing Requirements</h3>
            
            <p className="text-sm font-medium text-gray-700 mb-2">Type of Testing Required *</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-6">
              {['Load Testing (DG / UPS / Battery)', 'Battery Capacity / Discharge Testing', 'Power Quality Analysis (voltage, harmonics, frequency)', 'Thermography / Infrared Scanning (hotspot detection)', 'Earth / Ground Resistance Testing', 'Insulation Resistance Testing (Megger testing)', 'Short Circuit / Fault Level Testing', 'Cable Testing (continuity, insulation)', 'Panel & Switchgear Testing', 'Power Factor Testing', 'Energy Audit Testing', 'UPS Bypass Testing', 'ATS / Changeover Testing', 'Full Electrical Inspection', 'Not sure — need expert assessment'].map(s => (
                <label key={s} className="flex items-start gap-2 p-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" value={s} checked={formData.testingRequired.includes(s)} onChange={(e) => handleCheckboxChange(e, 'testingRequired')} className="mt-1 text-[#10AFA5] focus:ring-[#10AFA5] rounded" />
                  <span className="text-sm text-gray-700">{s}</span>
                </label>
              ))}
            </div>

            <p className="text-sm font-medium text-gray-700 mb-2">Equipment to be tested *</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {['DG Set', 'UPS System', 'Battery Bank', 'Main Distribution Board (MDB)', 'Sub Distribution Board (SDB)', 'Transformers', 'Capacitor Bank / APFC', 'Cables & Wiring', 'DC Power Plant', 'Solar System', 'Other'].map(s => (
                <label key={s} className="flex items-start gap-2 p-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" value={s} checked={formData.equipmentToTest.includes(s)} onChange={(e) => handleCheckboxChange(e, 'equipmentToTest')} className="mt-1 text-[#10AFA5] focus:ring-[#10AFA5] rounded" />
                  <span className="text-sm text-gray-700">{s}</span>
                </label>
              ))}
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiFileText className="text-[#10AFA5]" /> 3. Purpose of Testing</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="reasonForTesting" value={formData.reasonForTesting} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Reason for Testing *</option>
                {['New installation verification', 'Routine / periodic inspection', 'Compliance / regulatory requirement', 'Insurance / audit requirement', 'Before AMC renewal', 'After breakdown / repair', 'Performance issue investigation', 'Pre-commissioning check', 'Energy efficiency improvement', 'Other'].map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              <select required name="needDetailedReport" value={formData.needDetailedReport} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Need detailed test report? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <select required name="reportFormat" value={formData.reportFormat} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                ...
                {['PDF', 'Hard copy', 'Both'].map(f => <option key={f} value={f}>{f}</option>)}
              </select>
              <input required type="text" name="neededForAuthority" value={formData.neededForAuthority} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Needed for authority? (Yes-Specify / No) *" />
              <select required name="needRecommendations" value={formData.needRecommendations} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiClock className="text-[#10AFA5]" /> 4. Visit & Timeline</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
              ...
              <select required name="canTestDuringWorkingHours" value={formData.canTestDuringWorkingHours} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Can testing be done during working hours? *</option>
                <option value="Yes">Yes</option>
              ...
              <select required name="panelAccessAvailable" value={formData.panelAccessAvailable} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Electrical room / panel access available? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
              <select required name="projectUrgency" value={formData.projectUrgency} onChange={handleInputChange} className="w-full px-4 py-3 bg-[#E8F5F1] text-[#10AFA5] border border-[#10AFA5] font-bold rounded-xl outline-none">
                <option value="">Project Urgency *</option>
                {['ASAP', 'Within 1 week', 'Within 2 weeks', 'Flexible'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
              <input type="text" name="safetyRestrictions" value={formData.safetyRestrictions} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Any safety restrictions at site? (Describe or No)" />
            </div>
          </div>

          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiFileText className="text-[#10AFA5]" /> 5. Budget & Additional Info</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="budgetRange" value={formData.budgetRange} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                ...
                {['Below ₹5,000', '₹5,000–₹20,000', '₹20,000–₹50,000', 'Above ₹50,000', 'TBD'].map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="interestedInAmcAfter" value={formData.interestedInAmcAfter} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Interested in AMC after testing? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-2"><FiUploadCloud className="text-[#10AFA5]" /> 6. Attachments (optional)</h3>
            <p className="text-sm text-gray-500 mb-4">Upload Site SLD, Equipment List, Compliance Docs, etc.</p>
...
export default PowerTestingService;
