...
import { 
  ...
  FiMapPin, FiPrinter, FiTool, FiActivity, FiClock, FiFileText
} from 'react-icons/fi';
import toast from 'react-hot-toast';
import api from '../../../../services/api';
import BottomNav from '../../components/layout/BottomNav';

const PassbookPrinterService = () => {
  ...
  const [formData, setFormData] = useState({
    // Section 1
    fullName: '', organizationName: '', branchNameCode: '', designation: '', email: '', phone: '', city: '', branchAddress: '',
    // Section 2
    printerBrand: '', printerModelNumber: '', printerSerialNumber: '', numberOfPrinters: '', printerAge: '',
    // Section 3
    serviceRequired: '', printerStatus: '', errorShown: '', issueDescription: '',
    // Section 4
    lastServiced: '', existingAmc: '', amcProviderDetails: '', underWarranty: '', sparePartsNeeded: '', sparePartsDetails: '',
    // Section 5
    preferredVisitDate: '', preferredTimeSlot: '', urgency: '', siteContactName: '', siteContactNumber: '',
    // Section 6
    needServiceReport: '', interestedInAmc: '', budgetRange: '', source: '', additionalNotes: ''
  });
  ...
  // Dropdowns
  const printerBrands = ['Epson', 'TVS', 'Wincor / Diebold', 'NCR', 'Other (specify)'];
  const ages = ['Less than 1 year', '1–3 years', '3–5 years', 'More than 5 years'];
  const services = ['Breakdown / Not printing', 'Paper jam issue', 'Print quality issue (faded / skipping)', 'Passbook alignment issue', 'Roller / head replacement', 'Cleaning & maintenance', 'Ribbon / ink cartridge replacement', 'Software / driver issue', 'Preventive maintenance', 'New printer installation', 'AMC Renewal'];
  const statuses = ['Not working at all', 'Printing but poor quality', 'Paper jam stuck', 'Making noise but not printing', 'Working — routine maintenance'];
  const timeSlots = ['Morning 9–12', 'Afternoon 12–4', 'Evening 4–7'];
  const urgencies = ['Emergency — branch operations affected', 'Same day', 'Within 24 hours', 'Within 2–3 days', 'Flexible'];
  const budgets = ['To be discussed', 'Below ₹3,000', '₹3,000–₹8,000', 'Above ₹8,000'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/public/passbookprinter-enquiries', finalData);
      if (res.data.success) {
        setSuccess(true);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 pb-24">
        <div className="bg-white rounded-3xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-8">Our printer service team will contact you within 24 hours.</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center justify-between px-4 h-16">
          ...
          <h1 className="text-lg font-bold text-gray-800 truncate px-2">Passbook Printer Service</h1>
      ...
      <div className="bg-gradient-to-br from-[#E8F5F1] to-[#D1EBE5] px-6 py-8">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><FiPrinter className="text-2xl text-[#10AFA5]" /></div>
          <div>
            <h2 className="text-xl font-bold text-gray-800">Printer Maintenance</h2>
            <p className="text-gray-600 text-sm">Expert repair, service and parts replacement for passbook printers.</p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-[#E8F5F1] p-6 border-l-4 border-l-[#10AFA5]">
            ...
            <div className="space-y-4">
              ...
              <input required type="text" name="designation" value={formData.designation} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Designation (e.g. Branch Manager) *" />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input required type="text" name="branchAddress" value={formData.branchAddress} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Branch Address *" />
            </div>
          </div>

          {/* Section 2: Printer Details */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiPrinter className="text-[#10AFA5]" /> 2. Printer Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="printerBrand" value={formData.printerBrand} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Printer Brand *</option>
                {printerBrands.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
              <select required name="printerAge" value={formData.printerAge} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Printer Age *</option>
                {ages.map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              <input type="text" name="printerModelNumber" value={formData.printerModelNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Printer Model Number (optional)" />
              <input type="text" name="printerSerialNumber" value={formData.printerSerialNumber} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Printer Serial Number (optional)" />
              <input required type="number" name="numberOfPrinters" value={formData.numberOfPrinters} onChange={handleInputChange} className="w-full md:col-span-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Number of Printers at branch *" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              ...
              <select required name="printerStatus" value={formData.printerStatus} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Current Printer Status *</option>
                {statuses.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <textarea name="errorShown" value={formData.errorShown} onChange={handleInputChange} rows="2" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" placeholder="Error shown on printer / system (if any)"></textarea>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4 border-b pb-2"><FiActivity className="text-[#10AFA5]" /> 4. Maintenance History</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block px-1">Last serviced</label>
                <input type="date" name="lastServiced" value={formData.lastServiced} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none" />
                <button type="button" onClick={() => setFormData(p => ({ ...p, lastServiced: "Don't remember" }))} className="text-xs text-[#10AFA5] mt-1 ml-1 hover:underline">Set to "Don't remember"</button>
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block px-1">Under warranty?</label>
              ...
              <select required name="sparePartsNeeded" value={formData.sparePartsNeeded} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Spare parts needed? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
                <option value="Not sure">Not sure</option>
              </select>
              {formData.sparePartsNeeded === 'Yes' && (
                <input type="text" name="sparePartsDetails" value={formData.sparePartsDetails} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none animate-fade-in" placeholder="Which parts? (e.g. Ribbon / Roller) *" required />
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select required name="needServiceReport" value={formData.needServiceReport} onChange={handleInputChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#10AFA5] outline-none">
                <option value="">Do you need service report? *</option>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">Upload Printer Error Screenshots, Photos, or AMC/Warranty Documents.</p>
...
export default PassbookPrinterService;
