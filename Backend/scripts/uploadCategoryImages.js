...
const uploadCategoryImages = async () => {
  try {
    ...
    for (const category of categories) {
      ...
      // Upload category icon to WBI/{category}/icons/
      if (fs.existsSync(category.icon)) {
        ...
        const iconResult = await uploadFile(iconBuffer, {
          folder: `WBI/${category.name}/icons`,
        ...
        if (iconResult.success) {
          ...
          console.log(`  âŒ Icon upload failed: ${iconResult.error}`);
        }
      } else {
    ...
    const outputPath = path.join(process.cwd(), 'WBI-category-icons.json');
...
const runUpload = async () => {
  try {
    ...
    console.log('\nðŸŽ¯ Next step: Update seeding scripts with these WBI Cloudinary URLs');
  } catch (error) {
