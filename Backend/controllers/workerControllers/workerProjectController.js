...
/**
 * Get project counts by status for the authenticated worker
 */
const getProjectCounts = async (req, res) => {
  try {
    ...
    const [all, inProgress, onHold, completed] = await Promise.all([
      WorkerProject.countDocuments({ workerId }),
      WorkerProject.countDocuments({ workerId, status: 'In Progress' }),
      WorkerProject.countDocuments({ workerId, status: 'On Hold' }),
      WorkerProject.countDocuments({ workerId, status: 'Completed' })
    ]);
...
/**
 * Get list of projects for the authenticated worker
 */
...
const getProjectDocuments = async (req, res) => {
  try {
    ...
    const project = await WorkerProject.findOne({ _id: projectId, workerId }).select('documents');
...
const getProjectPayments = async (req, res) => {
  try {
    ...
    const project = await WorkerProject.findOne({ _id: projectId, workerId }).select('payments totalAmount paidAmount');
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });
...
const getProjectTimeline = async (req, res) => {
  try {
    ...
    const project = await WorkerProject.findOne({ _id: projectId, workerId }).select('timeline');
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });
...
const getProjectProgress = async (req, res) => {
  try {
    ...
    const project = await WorkerProject.findOne({ _id: projectId, workerId }).select('progress milestones');
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });
...
const submitMilestone = async (req, res) => {
  try {
    ...
    project.timeline.push({
      ...
      description: `Worker submitted milestone: ${milestone.title}`
    });
...
const getMilestoneById = async (req, res) => {
  try {
    ...
    const project = await WorkerProject.findOne({ _id: projectId, workerId }).select('milestones');
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });
