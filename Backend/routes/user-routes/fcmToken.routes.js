...
router.post('/save', authenticate, async (req, res) => {
  try {
    ...
    if (req.userRole !== 'USER') {
      return res.status(403).json({ success: false, error: `Access denied. You are logged in as a ${req.userRole}, but this route is for users.` });
    }
    ...
    */
    ...
    res.json({ success: true, message: 'FCM token saved successfully', role: 'user', platform });
  } catch (error) {
